...
void setup() {
  ...
  // 文字のバックの描画
  M5.Lcd.fillRect(0, 64, 160, 16, WHITE);
  // 文字の表示
  M5.Lcd.setTextColor(RED);
  M5.Lcd.setCursor(8 + 1, 64 + 1);
  ...
  M5.Lcd.print("Srv1");
  M5.Lcd.setCursor(2, 17);
  M5.Lcd.print("Srv2");
  M5.Lcd.setCursor(2, 33);
  M5.Lcd.print("Srv3");
  M5.Lcd.setCursor(2, 49);
  M5.Lcd.print("Srv4");
...
void loop() {
  // --Servo1 情報表示
  M5.Lcd.setCursor(62, 1);
  if (servo1.MemoryMap42to59()) {
    M5.Lcd.printf("%5.1fdeg", (float)(servo1.CurrentPosition) * 0.1);
  }
  else {
    M5.Lcd.print("N/A     ");
  }

  M5.Lcd.setCursor(62, 17);
  if (servo2.MemoryMap42to59()) {
    M5.Lcd.printf("%5.1fdeg", (float)(servo2.CurrentPosition) * 0.1);
  }
  else {
    M5.Lcd.print("N/A     ");
  }

  M5.Lcd.setCursor(62, 33);
  if (servo3.MemoryMap42to59()) {
    M5.Lcd.printf("%5.1fdeg", (float)(servo3.CurrentPosition) * 0.1);
  }
  else {
    M5.Lcd.print("N/A     ");
  }

  M5.Lcd.setCursor(62, 49);
  if (servo4.MemoryMap42to59()) {
    M5.Lcd.printf("%5.1fdeg", (float)(servo4.CurrentPosition) * 0.1);
  }
  else {
    M5.Lcd.print("N/A     ");
  }

  M5.Lcd.setCursor(62, 65);
  if (servo5.MemoryMap42to59()) {
    M5.Lcd.printf("%5.1fdeg", (float)(servo5.CurrentPosition) * 0.1);
  }
  else {
    M5.Lcd.print("N/A     ");
  }
